...
module.exports = (bot, { viewDistance = 6, firstPerson = false, port = 3000, follow = false }) => {
  ...
  bot.viewer.drawLine = (id, points, color = 0xff0000, offset=0) => {
  ...
  io.on('connection', (socket) => {
    socket.emit('version', bot.version,firstPerson,follow)
    sockets.push(socket)
	...
	socket.on('control', (flip) => {
		bot.controlled = flip;
		bot.clearControlStates();
	});
	
	socket.on('keydown', (keyP) => {
		keyPress(keyP,true);
	});
	
	socket.on('keyup', (keyP) => {
		keyPress(keyP,false);
	});
	
	socket.on('look', (vec) => {
		bot.entity.pitch+=vec[0]*config.viewer.viewSensitivity*0.001;
		bot.entity.yaw+=vec[1]*config.viewer.viewSensitivity*0.001;
	});
	
	socket.on('lookSet', (vec) => {
		bot.entity.pitch=vec[0];
		bot.entity.yaw=vec[1];
	});
