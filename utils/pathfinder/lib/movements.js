...
class Movements {
  constructor (bot, mcData) {
	...
	this.witherCost = function(block) {
		let cost = 0;
		let entity;
		for(entity in this.bot.entities){
			entity = bot.entities[entity];
			if( (entity.kind=='Hostile mobs' || entity.kind=='projectile') && block.position!=null){

				const distanceTo =block.position.distanceTo(entity.position) ;
				if(distanceTo<50){
				cost+=(50-distanceTo)*20;
				}
			}
		}
		...
		/*cost += bot.findBlock({
			...
			matching : function(block){ return(this.blocksToAvoid.has(block.type))},
			count : 20,
			maxDistance : 5,
		}).length*0.5;*/
    ...
    this.blocksToAvoid.add(139) // cobble walls (Weird collision
	this.blocksToAvoid.add(155) // just 2 be safe its a quartz block with pillar variations 
	this.blocksToAvoid.add(202) // purpur pillar? yeah FUCK that
  ...
  safeOrBreak (block, toBreak) {
    if (block.safe && block.name!='obsidian') return 0
    if (!this.safeToBreak(block)  && block.name!='obsidian') return 100 // Can't break, so can't move
    ...
    var digTime = block.digTime(tool ? tool.type : null, false, false, false, enchants, effects)
  ...
  getMoveWaterAbove (node, dir, neighbors) {
	 const blockC = this.getBlock(node, 0, 1, 0)
	 
	 if(blockAbove.liquid){
		let cost = 1;
		neighbors.push(new Move(blockC.position.x, blockC.position.y, blockC.position.z, 0, cost, [], []))
	 }
  }
  ...
  getLandingBlock (node, dir) {
	...
	if(this.bot.health >= 10){
		this.maxDropDown = Math.round(this.bot.health*0.5)
		if(this.bot.food<18){
			this.maxDropDown*0.7
			if(this.maxDropDown<3){
				this.maxDropDown=3;
			}
		}
	}else{
    ...
    for (let i = 0; i < 200 - 1; i++) {
      if (blockLand.physical || !blockLand.safe || blockLand.liquid) break
		const blockTest = this.getBlock(node, dir.x, -2 - i, dir.z);
		if(i<this.maxDropDown || blockTest.liquid){
			blockLand = blockTest;
		}else{
			if(blockTest.physical){
				break;
			}
		}
    }
  ...
  getMoveDropDown (node, dir, neighbors) {
    ...
    if (!blockLand.physical && !blockLand.liquid) return // TODO: place? (bridging down)
	...
	cost+=this.witherCost(blockLand);
  ...
  getMoveDown (node, neighbors) {
    ...
    if (!blockLand.physical && !blockLand.liquid) return
  ...
  getMoveParkourForward (node, dir, neighbors) {
    ...
    if (blockB.safe && blockC.safe && blockD.physical) {
      ...
      neighbors.push(new Move(blockC.position.x, blockC.position.y, blockC.position.z, node.remainingBlocks, 4, [], [], true))
    } else if (blockB.safe && blockC.physical) {
      // Up
      if (!this.getBlock(node, 0, 2, 0).safe ||
        !this.getBlock(node, dir.x, 2, dir.z).safe ||
        !this.getBlock(node, dx, 2, dz).safe) return
      neighbors.push(new Move(blockB.position.x, blockB.position.y, blockB.position.z, node.remainingBlocks, 4, [], [], true))
    } else if (blockC.safe && blockD.safe) {
      // Down
      const blockE = this.getBlock(node, dx, -2, dz)
      if (!blockE.physical && !blockE.liquid) return
      neighbors.push(new Move(blockE.position.x, blockE.position.y, blockE.position.z, node.remainingBlocks, 4, [], [], true))
    }
  }

  getMoveSuperParkourForward (node, dir, neighbors) {
    if (this.getBlock(node, dir.x, -1, dir.z).physical ||
      !this.getBlock(node, dir.x, 0, dir.z).safe ||
      !this.getBlock(node, dir.x, 1, dir.z).safe) return
    ...
    if (blockB.safe && blockC.safe && blockD.physical) {
      // Forward
      if (!this.getBlock(node, 0, 3, 0).safe ||
        !this.getBlock(node, dir.x, 3, dir.z).safe) return
      neighbors.push(new Move(blockC.position.x, blockC.position.y, blockC.position.z, node.remainingBlocks, 4, [], [], true))
    }  else if (blockC.safe && blockD.safe) {
      // Down
      const blockE = this.getBlock(node, dx, -3, dz)
      if (!blockE.physical && !blockE.liquid) return
      neighbors.push(new Move(blockE.position.x, blockE.position.y, blockE.position.z, node.remainingBlocks, 4, [], [], true))
    }
  }
  ...
  getNeighbors (node) {
    ...
    for (const i in cardinalDirections) {
      ...
      if (this.allowParkour) {
		//this.getMoveSuperSuperParkourForward(node, dir, neighbors)
        //this.getMoveSuperParkourForward(node, dir, neighbors)
        this.getMoveParkourForward(node, dir, neighbors)
	
      }
	  
	  if(this.bot.blockAt(new Vec3(node.x,node.y,node.z)).liquid){
		  this.getMoveWaterAbove(node, dir, neighbors);
	  }
    }
