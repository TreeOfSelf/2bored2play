...
var path = []
var bot = null;
var stateMovements = null
var stateGoal = null
var dynamicGoal = false
var pathUpdated = false
var digging = false
var placing = false
var placingBlock = null
var thinking = false
var lastNodeTime = performance.now()
var lastMovement = new Vec3(0,60000,0);



function inject (botInput) {
  bot = botInput;
  
  bot.pathfinder = {}

  bot.pathfinder.thinkTimeout = 50//config.ai.pathfindThinkInterval // ms
  ...
  bot.pathfinder.getPathTo = function (movements, goal, done, timeout) {
	...
	var count = movements.countScaffoldingItems();
    const start = new Move(p.x, p.y, p.z, count, 0)
  ...
  bot.on('chunkColumnLoad', (chunk) => {
		resetPath(config.ai.pauseWhileChunksLoading);
  })
  ...
  function monitorMovement () {
	 ...
	 if(bot.entity.isInWater){
		 if(path.length==0){
			...
			bot.setControlState('jump',path[path.length-1].y>=bot.entity.position.y-3 || path[0].y>=bot.entity.position.y-3); 
		 }
	 }
    ...
    if (stateMovements && stateMovements.allowFreeMotion && stateGoal && stateGoal.entity) {
      ...
      if (canStraightPathTo(target.position)) {
        ...
        if (target.position.distanceTo(bot.entity.position) > Math.sqrt(stateGoal.rangeSq)) {
		  ...
		  bot.setControlState('sprint',bot.sprint);
        } else {
          bot.clearControlStates()
        }
        return
    ...
    if (path.length === 0) {
      ...
      if (stateGoal && stateMovements && !thinking) {
		...
		//	if(stagnant>=2 && bot.targetDigBlock==null && checkInterval==null){
          thinking = true
       ...
       // }
		  ...
		  if(stateGoal == null && bot.goal!=null && (bot.entity.isInWater || bot.entity.onGround)){
			bot.emit('goal_reset');
		  }
	  }
      return
	...
	bot.setControlState('sprint',gottaJump);
